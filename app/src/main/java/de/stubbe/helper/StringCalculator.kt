...
class StringCalculator {
    ...
    enum class Operators {
        ...
        //Function to calculate with two number operation
        var calculationTwoNumber: ((Double, Double) -> Double)? = null

        //Function to calculate with one number operation
        var calculationOneNumber: ((Double) -> Double)? = null
    ...
    enum class Variables {
        ...
        constructor(sign: String, value: Double) {
            this.sign = sign
            this.value = value
        }
    ...
    companion object Calculator {
        ...
        private fun calculateSplitList(list: MutableList<String>): Double {
            ...
            val index = Operators.operatorPositionWithHighestPriority(listToEdit)
        ...
        /**Calculates the given numbers by an operator */
        private fun calculateValues(list: MutableList<String>, indexOfOperator: Int): Double? {
            var result: Double? = null

            //Get the calculation from the operation and calculate the result
            val operation = Operators.getOperatorBySign(list[indexOfOperator])!!
            ...
            when (operation.calculationTyp) {
                Operators.CalculationTyp.LeftAndRight -> {
                    ...
                    if (indexOfOperator-1 == -1 || getNumber(list[indexOfOperator - 1], 0) == null) {
                        if (list[indexOfOperator] == "-")
                            result = (list[indexOfOperator] + list[indexOfOperator+1]).toDouble()
                        else if (Operators.isRightAndLeftOperator(list[indexOfOperator].first()))
                            result = list[indexOfOperator+1].toDouble()

                        //Replace
                        replaceElements(list, (indexOfOperator) .. (indexOfOperator + 1), result.toString())

                        return result
                    }

                    val value1 = list[indexOfOperator - 1].toDouble()
                    val value2 = list[indexOfOperator + 1].toDouble()

                    //Calculates the result with left and right number by operator
                    result = operation.calculationTwoNumber?.invoke(value1, value2)

                    debug("Calculation", listOf(Pair("Typ", operation.name), Pair("Expression", "$value1 ${operation.sign} $value2"),
                        Pair("Sub result", "$result")))

                    //Replace the calculated calculation with the result
                    replaceElements(list, (indexOfOperator-1) .. (indexOfOperator + 1), result.toString())
                }
                ...
                Operators.CalculationTyp.Left -> {
                    ...
                    try {
                        value = list[indexOfOperator - 1].toDouble()
                    } catch (e:Exception) {
                        error("No valid input! A value is missing.")
                    }

                    //Calculates the result with right number by operator
                    result = operation.calculationOneNumber?.invoke(value)

                    debug("Calculation", listOf(Pair("Typ", operation.name), Pair("Expression", "${operation.sign} $value"),
                        Pair("Sub result", "$result")))

                    //Replace the calculated calculation with the result
                    replaceElements(list, (indexOfOperator-1) .. (indexOfOperator), result.toString())
                }
        ...
        fun splitStringCalculation(string: String): MutableList<String> {
            ...
            fun addOperator() {
                ...
                if (i-1 >= 0 && (Operators.getOperatorBySign(operator)!!.calculationTyp == Operators.CalculationTyp.Right &&
            ...
            while (i < string.length) {
                ...
                else if (Variables.isVariable(getVariable(string, i))) {
                    addVariable()
                }
            ...
            //Remove all operators and brackets if they is no value to it
        ...
        private fun getCalculationInsideBrackets(list: MutableList<String>, index: Int): MutableList<String> {
        ...
        private fun getCalculationInsideBrackets(string: String, index: Int): String {
        ...
        private fun getOperator(string: String, index: Int): String {
            ...
            if (letters.first.isNotEmpty()) {
                if (Operators.isRightAndLeftOperator(letters.first[letters.second])) {
                    operator = letters.first[letters.second].toString()
                } else {
                    for (s in Operators.getAllOperatorSigns()) {
        ...
        /**Check if any valid operator is contained in char */
        fun isAnyOperator(parentString: String, posInParent: Int): Boolean {
            //Check if parent String is number or bracket and return false
            return Operators.getOperatorBySign(getOperator(parentString, posInParent)) != null
        }
